# QNAP iSCSI dynamic provisioning (management-hub)
...
| File | Purpose |
|------|---------|
| `externalsecret.yaml` | Pulls QNAP mgmt creds (`qnap-csi`) + per-cluster CHAP/portal (`management-hub-qnap-iscsi`) from 1Password into the `qnap-trident-backend-creds` Secret in the `trident` namespace |
| `trident-backend.yaml` | `TridentBackendConfig` (driver `qnap-iscsi`, CHAP, `location=management-hub` label) |
| `storageclass.yaml` | `qnap-iscsi` StorageClass selecting `location=management-hub`, non-default |
...
4. **1Password items** (Dev vault):
   ...
   - `management-hub-qnap-iscsi` — `storageAddress` (QNAP portal IP),
